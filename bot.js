...
// ===== EXTRACT MESSAGE TEXT (ALL cases including mentions) =====
function extractMessageText(msg) {
    try {
        ...
        // 2. Extended text (mentions, links, etc)
        else if (message.extendedTextMessage) {
            text = message.extendedTextMessage.text || '';

            // Also check contextInfo for mentions
            const context = message.extendedTextMessage.contextInfo;
            if (context) {
                // Check quoted message
                if (context.quotedMessage) {
                    const qm = context.quotedMessage;
                    if (qm.conversation) text += ' ' + qm.conversation;
                    else if (qm.extendedTextMessage?.text) text += ' ' + qm.extendedTextMessage.text;
                }
                // If mentions exist, add flag
...
async function startBot() {
    ...
    sock.ev.on('messages.upsert', async (m) => {
        ...
        if (hasStatus) {
            console.log(`🚨 STATUS KEYWORD FOUND! Attempting delete...`);
        ...
        if (hasBlood) {
            console.log(`🩸 BLOOD REQUEST FOUND! Sending reply...`);
