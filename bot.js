...
// ===== EXTRACT MESSAGE TEXT (ALL cases) =====
function extractMessageText(msg) {
    try {
        ...
        // Case 2: Extended text (mentions, forwards, links)
            ...
            // Check quoted/forwarded message text too
...
async function startBot() {
    ...
    sock.ev.on('messages.upsert', async (m) => {
        ...
        console.log(`\n📩 New msg | From: ${senderNumber} | Text: ${text.substring(0, 60)}`);

        // ----- FEATURE 1: Auto-delete status -----
        const hasStatus = config.statusKeywords.some(k => lowerText.includes(k.toLowerCase()));

        if (hasStatus) {
            console.log(`🚨 STATUS FOUND! Trying to delete...`);

            try {
                // DIRECT DELETE - no admin check!
                await sock.sendMessage(chatId, { delete: msg.key });
                console.log('   ✅ DELETED!');

                // Send warning after 1.5 sec
                setTimeout(async () => {
                    try {
        ...
        if (hasBlood) {
            console.log(`🩸 BLOOD FOUND! Sending reply...`);
