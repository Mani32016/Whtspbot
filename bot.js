...
function extractMessageText(msg) {
    try {
        ...
        // Case 2: Extended text (mentions, forwards, links)
        else if (msg.message.extendedTextMessage) {
            text = msg.message.extendedTextMessage.text || '';

            // Check if it's a forwarded status or quoted message
            const context = msg.message.extendedTextMessage.contextInfo;
            if (context) {
                // If there's a quoted message, check its text too
                if (context.quotedMessage) {
                    const quotedType = getContentType(context.quotedMessage);
                    if (quotedType === 'conversation') {
                        text += ' ' + (context.quotedMessage.conversation || '');
                    } else if (quotedType === 'extendedTextMessage') {
                        text += ' ' + (context.quotedMessage.extendedTextMessage?.text || '');
                    }
                }
                // Check mentioned JIDs (if someone is mentioned)
                if (context.mentionedJid && context.mentionedJid.length > 0) {
                    text += ' mention';
                }
            }
        }
        // Case 3: Image/video with caption
...
async function isBotAdmin(chatId) {
    try {
        ...
        const me = botNumber || sock.user.id;
        const participant = metadata.participants.find(p => p.id === me);
...
async function startBot() {
    ...
    sock.ev.on('connection.update', (update) => {
        ...
        if (connection === 'close') {
            ...
            console.log('   • Auto-delete status mentions/shares');
            console.log('   • Auto-reply blood requests');
            console.log('   • Multi-group support');
            console.log('=====================================\n');
            console.log('💾 IMPORTANT: Visit /download-auth to save session!\n');
        }
    });
    ...
    sock.ev.on('messages.upsert', async (m) => {
        ...
        // ----- FEATURE 1: Auto-delete status mentions/shares -----
        ...
        if (hasStatus) {
            ...
            if (admin) {
                ...
                console.log('   ⚠️ Bot is NOT admin - sending warning');
                const warnMsg = config.statusDeleteFailMessage.replace('{{user}}', senderNumber);
                await sock.sendMessage(chatId, { text: warnMsg });
            }
            return;
        ...
        if (hasBlood) {
            console.log(`🩸 BLOOD REQUEST DETECTED!`);
            console.log(`   Group: ${chatId}`);
            console.log(`   From: ${senderNumber}`);
