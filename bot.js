...
async function startBot() {
    ...
    // ===== QR CODE AS CLICKABLE URL =====
    sock.ev.on('connection.update', (update) => {
        const { connection, lastDisconnect, qr } = update;

        if (qr) {
            qrCount++;
            // Generate a clickable QR code image URL!
            const qrImageUrl = `https://api.qrserver.com/v1/create-qr-code/?size=400x400&data=${encodeURIComponent(qr)}`;

            console.log(`\n========================================`);
            console.log(`🔷 QR CODE #${qrCount}`);
            console.log(`📱 Is link ko click karein ya copy karke browser mein kholein:`);
            console.log(`\n${qrImageUrl}\n`);
            console.log(`👉 Link khulne ke baad QR Code image scan karein!`);
            console.log(`⏳ Ye link 20 second mein expire ho jayegi...`);
            console.log(`========================================\n`);
        }
